...
class ThirdWin(tk.Toplevel, App, BankAccount):
    ...
    def depositFrame(self):
        ...
        self.depBtn = ttk.Button(self.thirdWin, text="Deposit", command=partial(ThirdWin.subDeposit, self))
        self.depBtn.place(x=150, y=200)
        
    def subDeposit(self):
        print(f'Deposited ${float(self.depEntry.get())}')
        self.bank.balance += float(self.depEntry.get())
        print(f'Balance ${self.bank.getBalance()}')
        self.depEntry.delete(0, END)
        
    def goHome(self):
        self.depLabel.destroy()
        self.depEntry.destroy()
        self.depBtn.destroy()
    ...
    def gridFrame(self):
        ...
        self.withBtn = ttk.Button(self.frame, text="Withdraw")#, height=4, width=12)
